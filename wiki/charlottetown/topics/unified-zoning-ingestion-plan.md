...
- Section-equivalence candidate generation has started. `scripts/generate-charlottetown-section-equivalence.py` now populates 137 tuned `title_topic_token_v1` rows in `zoning.section_equivalence` after exact-title and weak-false-positive controls were checked on 2026-04-29. Manual review has recorded accepted ledger decisions for all 76 `same_topic` matches; 35 are already applied in the database and 41 remain pending database update.
...
3. Active next milestone: add review-status reporting and continue manual review for comparable current and draft bylaw sections. The first candidate method, `title_topic_token_v1`, is implemented, tuned, populated, and the `same_topic` candidate group has accepted decisions recorded in the ledger.

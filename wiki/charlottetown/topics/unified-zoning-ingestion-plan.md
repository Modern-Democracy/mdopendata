...
- Section-equivalence candidate generation has started. `scripts/generate-charlottetown-section-equivalence.py` now populates 137 tuned `title_topic_token_v1` rows in `zoning.section_equivalence` after exact-title and weak-false-positive controls were checked on 2026-04-29. Manual review has accepted 35 exact-title `same_topic` matches, leaving 102 candidate rows for continued review.
...
3. Active next milestone: add review-status reporting and continue manual review for comparable current and draft bylaw sections. The first candidate method, `title_topic_token_v1`, is implemented, tuned, populated, and partially reviewed.

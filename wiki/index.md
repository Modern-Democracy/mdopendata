...
| [Render and Supabase demonstration deployment](./implementation/render-supabase-deployment.md) | Canonical read-only demonstration endpoint at `https://mdopendata-demo.onrender.com`, backed by a one-time Supabase PostGIS snapshot, with local ingestion retained outside the remote environment. |
...
| [Budget content and observation redesign status](./budgets/content-and-observation-redesign-status.md) | Breaking numeric fact rename, canonical three-edition hierarchy, contextual extraction, appendix recovery, redesigned routes, and QA evidence. |
...
| [Agenda package ingestion contract](./implementation/agenda-package-ingestion-contract.md) | Package JSON, logical-document ordering, template discovery, and the approved Charlottetown public-meeting reuse profile and controls. |
...
| [Staged PDF artifact version 2 implementation plan](./implementation/staged-pdf-artifact-v2-implementation-plan.md) | Implementation sequence through the completed Phase 7 parity and active-workspace handoff, with remaining Phase 6 agenda-profile work explicit. |
| [Charlottetown staged PDF shadow pilot](./implementation/staged-pdf-inventory-extraction-charlottetown-pilot.md) | Frozen version 1 baseline, active version 2 evidence, Stage 2 groups and observations, Snapshot 3 parity, and rollback boundary. |
| [Staged PDF inventory review UI](./implementation/staged-pdf-inventory-review-ui-plan.md) | Loopback-only active version 2 review and explicit version 1 rollback, including spans, table titles, propagation, immutable templates, review policies, and parity. |

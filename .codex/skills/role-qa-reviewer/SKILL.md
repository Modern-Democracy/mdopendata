...
Validate at the earliest reliable source level. Prefer raw source data, parser inputs, database records, or canonical controls over derived outputs that may add noise.

When two outputs should agree, compare stable common features directly. Use extents, counts, centroids, or visual inspection only as supporting evidence unless they test the invariant.
...
Prefer falsifiable thresholds tied to the task, such as overlap ratios, residual distances, unmatched counts, schema violations, round-trip differences, or exact equality counts.

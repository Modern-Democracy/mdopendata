...
Route dependency additions or updates, installations, runtimes, toolchains, environment configuration, containers, CI/CD, infrastructure, deployment targets, and all other DevOps work to `DevOps`. Other roles may identify the need, but only `DevOps` may perform these mutations after its user-approval gate.
...
- Do not permit another role to install or update dependencies, mutate project environments, or change deployment configuration. Route that work to `DevOps`.
...
- route to `DevOps` before any dependency, environment, toolchain, container, infrastructure, CI/CD, or deployment mutation

...
- refreshed Phase 4 ledger counts are 38 `numeric_value_review` rows, 29 `extraction_review` rows, 24 `table_parsing_review` rows, 4 `schedule_map_review` rows, 0 `section_assignment_review` rows, and 3 `layout_order_review` rows

Next actions:

1. Triage the remaining lower-density numeric table-cell flags.

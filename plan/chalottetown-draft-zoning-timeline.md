...
- refreshed Phase 4 ledger counts are 0 `numeric_value_review` rows, 29 `extraction_review` rows, 24 `table_parsing_review` rows, 4 `schedule_map_review` rows, 0 `section_assignment_review` rows, and 3 `layout_order_review` rows

Next actions:

1. Continue broad extraction and table-parsing review only where source-page inspection identifies a concrete wrong-section, wrong-order, figure-bleed, or table-placement defect.
2. Continue broad extraction and table-parsing review only where source-page inspection identifies a concrete wrong-section, wrong-order, figure-bleed, or table-placement defect.

...
1. Review the next highest-density `needs_review` files: `zones/hi.json`, `general-provisions-subdividing-land.json`, `permit-applications-processes.json`, `zones/dn.json`, `zones/p.json`, and `zones/rm.json`.
2. Review the remaining open non-clause rows in the four zone files only if they still affect Phase 3 acceptance.

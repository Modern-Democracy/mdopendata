...
SOURCE_PDF = ROOT / "docs" / "eastern-passage-cow-bay-land-use-bylaw.pdf"
OUTPUT_ROOT = ROOT / "data" / "zoning" / "eastern-passage-cow-bay"

JURISDICTION = "Halifax Regional Municipality"
BYLAW_NAME = "Land Use By-law for Eastern Passage/Cow Bay"
SOURCE_DOCUMENT_PATH = "docs/eastern-passage-cow-bay-land-use-bylaw.pdf"

BYLAW_PAGE_RE = re.compile(r"Eastern Passage/Cow Bay Land Use By-law Page\s+(\d+)", re.IGNORECASE)
...
def merge_same_heading_ranges(
    ranges: list[tuple[int, int, re.Match[str]]],
) -> list[tuple[int, int, re.Match[str]]]:
    merged: list[tuple[int, int, re.Match[str]]] = []
    for start, end, match in ranges:
        if not merged:
            merged.append((start, end, match))
            continue
        prev_start, prev_end, prev_match = merged[-1]
        if compact_space(prev_match.group(1)).lower() == compact_space(match.group(1)).lower() and start <= prev_end + 1:
            merged[-1] = (prev_start, end, prev_match)
            continue
        merged.append((start, end, match))
    return merged
...
def parse_numbered_sections(rows: list[tuple[dict, str]]) -> tuple[list[dict], set[str]]:
    ...
    for page, line in rows:
        ...
        if match and not line.startswith("("):
            ...
            if re.fullmatch(r"\d+\.\d+", label) and title[:1].islower():
                if current is not None:
                    current["text_parts"].append(line)
...
def should_merge_use_line(previous: str, current: str) -> bool:
    ...
    if current_clean[:1].isdigit():
        return True
    first_word = current_clean.split()[0].rstrip(",;:.")
...
def infer_zone_code_and_name(part_title: str) -> tuple[str, str]:
    ...
    cleaned = re.sub(r"\s+\((?:RC|NWCC|HECC|Special|Task Force|Municipal Affairs|C|M)[^)]*\)\s*$", "", cleaned)
    if cleaned.upper().endswith("ZONE"):
        cleaned = cleaned[:-4].strip()
    cleaned = re.sub(r"^\([^)]*\)\s*", "", cleaned)
    cleaned = cleaned.replace("R-l", "R-1").replace("C-l", "C-1").replace("I-l", "I-1").replace("P-l", "P-1").replace("D-l", "D-1")

    match = re.match(r"^(?:\d{4}\s+)?([A-Z0-9-]+)\s+\((.*?)\)\s*(.*?)$", cleaned)
    if match:
        zone_code = normalize_zone_code(match.group(1))
    ...
    if all_codes:
        zone_code = normalize_zone_code(all_codes[-1])
        zone_name = compact_space(re.sub(r"\([A-Z0-9-]+\)\s*$", "", cleaned).strip(" -"))
        return zone_code, zone_name

    tokens = cleaned.split()
    zone_code = normalize_zone_code(tokens[-1])
...
def build_appendix_payload(pages: list[dict], start: int, end: int, match: re.Match[str]) -> tuple[str, dict, dict | None]:
    ...
    is_map_plate = not sections and (
        len(text) < 220
        or line_count < 12
        or any(token in lowered_title for token in ("travelled ways", "silver sands", "zoning map"))
    )
    slug = slugify(label)
    metadata = {
        "jurisdiction": JURISDICTION,
...
def build_schedule_payload(pages: list[dict], start: int, end: int, match: re.Match[str]) -> tuple[str, dict, dict | None]:
    ...
    is_map_plate = not sections and (
        ...
        or any(
            ...
            for token in ("wetland", "archaeological", "wind energy", "bonus zoning", "heritage hills", "zoning")
        )
    )
    metadata = {
        "jurisdiction": JURISDICTION,
    ...
    if is_map_plate:
        map_reference = {
            ...
            "source_label_raw": f"{label}: {title}" if title else label,
            "feature_key": slugify(f"{label} {title or ''}"),
            "feature_class": "wetland_area" if "wetland" in lowered_title else ("archaeological_constraint_area" if "archaeological" in lowered_title else ("wind_energy_area" if "wind" in lowered_title else ("bonus_zoning_area" if "bonus zoning" in lowered_title else ("zoning_boundary" if "zoning" in lowered_title else "site_specific_area")))),
            "pdf_page_start": pages[start]["pdf_page"],
            "pdf_page_end": pages[end]["pdf_page"],
            "bylaw_page_start": pages[start]["bylaw_page"],
            "bylaw_page_end": pages[end]["bylaw_page"],
...
def main() -> None:
    ...
    definitions_end = page_index_for_bylaw_page(pages, 15)
    general_start = page_index_for_bylaw_page(pages, 20)
    general_end = page_index_for_bylaw_page(pages, 43)
    signs_start = page_index_for_bylaw_page(pages, 44)
    signs_end = page_index_for_bylaw_page(pages, 47)
    zones_start = page_index_for_bylaw_page(pages, 48)
    zones_end = page_index_for_bylaw_page(pages, 104)
    administration_start = page_index_for_bylaw_page(pages, 105)
    administration_end = page_index_for_bylaw_page(pages, 106)
    appendix_start = page_index_for_bylaw_page(pages, 107)
    appendix_end = page_index_for_bylaw_page(pages, 120)
    schedule_start = page_index_for_bylaw_page(pages, 121)
    schedule_end = page_index_for_bylaw_page(pages, 126)
    ...
    maps: list[dict] = []
    write_json(
        OUTPUT_ROOT / "schedules" / "schedule-a.json",
        {
            "schedule_metadata": {
                "jurisdiction": JURISDICTION,
                "bylaw_name": BYLAW_NAME,
                "source_document_path": SOURCE_DOCUMENT_PATH,
                "schedule_label_raw": "Schedule A",
                "title_label_raw": "Eastern Passage / Cow Bay Zoning Map",
                "document_type": "schedule_map_plate",
                "status": "active",
                "pdf_page_start": pages[6]["pdf_page"],
                "pdf_page_end": pages[6]["pdf_page"],
                "bylaw_page_start": pages[6]["bylaw_page"],
                "bylaw_page_end": pages[6]["bylaw_page"],
            },
            "map_reference": {
                "source_label_raw": "Schedule A: Eastern Passage / Cow Bay Zoning Map",
                "feature_key": "schedule-a-eastern-passage-cow-bay-zoning-map",
    ...
    maps.append(
        {
            ...
            "bylaw_page_end": pages[6]["bylaw_page"],
            "planned_postgis_target": "spatial_features.geom",
            "schedule_file": "schedules/schedule-a.json",
        }
    )
    ...
    for start, end, match in merge_same_heading_ranges(page_ranges_by_heading(pages, schedule_start, schedule_end, SCHEDULE_RE)):

    ...
    for document in DOCUMENTS:
        ...
        for candidate in candidates:
            ...
            for row in source_rows:
                ...
                if candidate["table_family"] == "facility_operating_statement" and not values and label in {"Operating Revenue", "Operating Grants", "Operating Expenses"}:
                    facility_group_label = label
                if approved_profile:
                    ...
                    if document == "2025-2026" and candidate["table_family"] == "operating_detail":
                        detail_shape_approved = len(values) == (3 if candidate["page_start"] in {17, 18} else 1)
                    approved = (
                        bool(label)
                    ...
                    elif candidate["page_start"] == 145 and len(values) == 1 and (label.startswith("Total ") or label in {"InLieu of Property Taxes", "Municipal Support Grant", "Partial Grant In Lieu to Taxes (QEH)"}):
                        raw_value = values[-1]
                        mapped_rows.append({"row_id": row["row_id"], "raw_label": label, "raw_text": row["trimmed_text"], "source_value_ids": row["value_ids"], "source_values": values, "row_semantics": "total" if label.startswith("Total ") else "detail", "aggregation_role": "total" if label.startswith("Total ") else "detail", "reporting_entity_key": "city-of-charlottetown", "facts": [{"source_value_id": raw_value["value_id"], "document_period_key": "2025-2026-budget", "amount_type": "budget", "measure_unit": "CAD", "value_state": "reported_value", "numeric_value": raw_value["parsed_decimal"]}], "review_status": "approved"})
                    ...
                    approved = bool(label) and [value["value_index"] for value in values] == list(range(1, len(values) + 1)) and all(value["value_index"] in periods for value in values)

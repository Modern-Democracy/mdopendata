...
SOURCE_PDF = ROOT / "docs" / "cole-harbour-westphal-municipal-planning-strategy.pdf"
OUTPUT_ROOT = ROOT / "data" / "municipal-planning-strategy" / "cole-harbour-westphal"
...
SECTION_SPECS = [
    ...
    SectionSpec("regional-context", "SECTION I", "REGIONAL CONTEXT", 10, 13, "citywide"),
    SectionSpec("community-profile", "SECTION I", "COMMUNITY PROFILE", 14, 18, "citywide"),
    SectionSpec("issues", "SECTION I", "ISSUES", 19, 21, "citywide"),
    SectionSpec(
    ...
    SectionSpec("transportation", "SECTION II", "TRANSPORTATION", 43, 48, "citywide"),
    SectionSpec("heritage-conservation", "SECTION II", "HERITAGE CONSERVATION", 49, 49, "citywide"),
    SectionSpec("recreation", "SECTION II", "RECREATION", 50, 51, "citywide"),
    SectionSpec("education", "SECTION II", "EDUCATION", 52, 53, "citywide"),
    SectionSpec(
    ...
    SectionSpec("social-housing", "SECTION II", "SOCIAL HOUSING", 56, 56, "citywide"),
    SectionSpec("land-use-intent", "SECTION III", "LAND USE INTENT", 57, 59, "citywide"),
    SectionSpec(
    ...
    SectionSpec(
        ...
        "SECTION III",
        "MORRIS-RUSSELL LAKE SECONDARY PLANNING STRATEGY (RC-Mar 22/05;E-Apr 23/05)",
        80,
        94,
    ...
    SectionSpec("special-area-designation", "SECTION III", "SPECIAL AREA DESIGNATION", 108, 109, "citywide"),
    SectionSpec("watershed-designation", "SECTION III", "WATERSHED DESIGNATION", 110, 111, "citywide"),
    SectionSpec("implementation", "IMPLEMENTATION", "IMPLEMENTATION", 112, 120, "implementation"),
    SectionSpec(
    ...
    SectionSpec("amendment-index", "AMENDMENT INDEX", "AMENDMENT INDEX", 121, 123, "appendix"),
    SectionSpec(
        "schedule-a-mixed-residential-development-in-highway",
        "Schedule A",
        "Schedule A: Mixed Residential Development in Highway (RC-Oct 7/14;E-Nov 8/14)",
        124,
        124,
        "appendix",
    ),
...
MAP_FEATURE_CLASS_RULES = [
    ...
    ("planning_area_boundary", ["servicing boundaries", "secondary planning strategy area", "watershed boundaries"]),
    ("transportation_corridor", ["transportation", "urban road classification"]),
    ("watershed_area", ["watershed"]),
    ("municipal_service_area", ["water service district", "servicing boundaries"]),
    ("development_sub_area", ["sub area"]),
]
...
SECTION_POLICY_PREFIXES = {
    ...
    "rural-residential-designation": {"RR"},
    "community-commercial-designation": {"CC"},
    "highway-commercial-designation": {"HC"},
    "mixed-residential-development-in-the-highway-commercial-designation": {"HC"},
    "community-facility-designation": {"CF"},
    "provincial-park-designation": {"PP"},
    "special-area-designation": {"SA"},
    "watershed-designation": {"W"},
    "implementation": {"IM"},
}
...
def extract_lines(reader: PdfReader) -> list[dict]:
    ...
    for pdf_page, page in enumerate(reader.pages, start=1):
        ...
        for raw_line in raw_text.splitlines():
            ...
            if not line:
                continue
            if re.match(r"^Cole Harbour/Westphal Municipal Planning Strategy Page \d+$", line):
                continue
            if re.fullmatch(r"Page \d+", line):
                continue
            if line.startswith("APPENDIX A") and pdf_page == 119:
        ...
        entries.append(
            {
                ...
                "reason": (
                    "Referenced by the Cole Harbour/Westphal Municipal Planning Strategy and "
...
def section_payload(spec: SectionSpec, parsed: dict) -> dict:
    return {
        "section_metadata": {
            ...
            "document_name": "Cole Harbour/Westphal Municipal Planning Strategy",
...
def main() -> None:
    ...
    all_open_issues: list[dict] = [
        {
            "issue_type": "identifier_normalization_pending_review",
            "example_labels_raw": ["E-10(b)", "UR-32J", "Policy CC-9a", "IM-9(a)(xiii)"],
            "reason": "MPS mixed-format policy identifiers are not approved for normalized hierarchy handling in this repository.",
        },
        {

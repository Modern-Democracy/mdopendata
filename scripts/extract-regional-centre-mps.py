...
SOURCE_PDF = ROOT / "docs" / "regional-centre-municipal-planning-strategy.pdf"
OUTPUT_ROOT = ROOT / "data" / "municipal-planning-strategy" / "regional-centre"
SECTION_DIR = OUTPUT_ROOT / "sections"
PDF_TO_MPS_PAGE_OFFSET = 0
DOCUMENT_NAME = "Regional Centre Secondary Municipal Planning Strategy"
...
SECTION_SPECS = [
    SectionSpec("part-1-introduction", "PART 1", "INTRODUCTION", 14, 38, "introduction"),
    SectionSpec("part-2-urban-structure", "PART 2", "URBAN STRUCTURE", 39, 118, "secondary_planning_strategy"),
    SectionSpec("part-3-urban-design", "PART 3", "URBAN DESIGN", 119, 144, "secondary_planning_strategy"),
    SectionSpec(
        "part-4-culture-and-heritage-resources",
        "PART 4",
    ...
    SectionSpec("part-5-housing", "PART 5", "HOUSING", 162, 169, "secondary_planning_strategy"),
    SectionSpec(
    ...
    SectionSpec("part-7-mobility", "PART 7", "MOBILITY", 176, 185, "secondary_planning_strategy"),
    SectionSpec("part-8-environment", "PART 8", "ENVIRONMENT", 186, 192, "secondary_planning_strategy"),
    SectionSpec("part-9-implementation", "PART 9", "IMPLEMENTATION", 193, 226, "implementation"),
    SectionSpec("appendix-1-key-performance-indicators", "APPENDIX 1", "KEY PERFORMANCE INDICATORS", 227, 233, "appendix"),
    SectionSpec("appendix-2-regional-centre-urban-design-manual", "APPENDIX 2", "REGIONAL CENTRE URBAN DESIGN MANUAL", 234, 234, "appendix"),
    SectionSpec("amendments", "AMENDMENTS", "REGIONAL CENTRE SECONDARY MUNICIPAL PLANNING STRATEGY AMENDMENTS", 235, 236, "appendix"),
]


MAP_FEATURE_CLASS_RULES = [
    ("future_land_use_area", ["urban structure designations", "land use concept", "land use designation"]),
    ("planning_area_boundary", ["planning areas", "area plan boundary", "study area"]),
    ("site_specific_plan_area", ["special area", "central blocks"]),
    ("environmental_constraint_area", ["environmental sensitivity", "shadow impact"]),
    ("flood_plain_area", ["flood plain", "floodplains"]),
    ("watershed_area", ["watershed"]),
    ("transportation_corridor", ["transportation reserve", "pedestrian-oriented commercial streets"]),
    ("view_plane_area", ["view planes", "sight lines", "view corridors", "view terminus", "view lines"]),
    ("heritage_area", ["heritage conservation district", "heritage conservation districts"]),
    ("development_sub_area", ["development areas", "development sub-areas", "precincts", "maximum floor area", "maximum building height"]),
...
POLICY_CODE_RE = re.compile(r"^([A-Z]{1,6}(?:/[A-Z]{1,6})?-\d+[A-Z]*(?:\.[0-9A-Za-z]+)*(?:\([A-Za-z0-9.]+\))*)(?:\s+(.*))?$")
INLINE_POLICY_CODE_RE = re.compile(r"([A-Z]{1,6}(?:/[A-Z]{1,6})?-\d+[A-Z]*(?:\.[0-9A-Za-z]+)*(?:\([A-Za-z0-9.]+\))*)\s+")
...
def split_embedded_policy_code_lines(line: str) -> list[str]:
    if line.startswith(("Policy ", "Policies ")):
        return [line]
    starts: list[int] = []
...
def extract_lines(reader: PdfReader) -> list[dict]:
    ...
    for pdf_page, page in enumerate(reader.pages, start=1):
        ...
        for raw_line in raw_text.splitlines():
            ...
            if not line:
                continue
            if re.fullmatch(rf"{re.escape(DOCUMENT_NAME.upper())}\s+\|\s+\d+", line):
                continue
...
def parse_listed_map_references(lines: list[dict]) -> list[dict]:
    maps: list[dict] = []
    front_matter = SectionSpec(
        "front-matter-list-of-maps",
        "LIST OF MAPS",
        "LIST OF MAPS",
        10,
        10,
    ...
    for row in lines:
        if row["pdf_page"] == 10 and MAP_CAPTION_RE.match(row["text"]):
            maps.append(parse_map_reference(row, front_matter))
    return maps
...
def parse_section_content(spec: SectionSpec, rows: list[dict]) -> dict:
    if spec.section_type == "appendix":
        return {
            "context_blocks": [
                {
                    "text": compact_space(" ".join(row["text"] for row in rows)),
                    "topic": None,
    ...
    for index, row in enumerate(rows):
        ...
        if POLICY_NAMED_RE.match(text):
            ...
            if (
                current_record
                and current_record["record_kind"] == "policy"
                and normalize_policy_reference(label) == normalize_policy_reference(current_record["label_raw"])
            ):
                current_record["lines"].append({**row, "text": rest})
                continue
            flush_context()
            flush_record()
...
def spatial_backlog_entries(refs: list[dict], spec: SectionSpec) -> list[dict]:
    ...
    for ref in refs:
        reason = (
            "Referenced by the Regional Centre Secondary Municipal Planning Strategy and required for "
...
def section_payload(spec: SectionSpec, parsed: dict) -> dict:
    return {
        ...
        "normalization_policy": {
            ...
            "reason": "MPS policy identifiers, including dotted identifiers such as IM-33.5, are single code values and are preserved raw.",
        },
        **parsed,
    }
...
def document_payload(reader: PdfReader, sections: list[dict], maps: list[dict], backlog: list[dict]) -> dict:
    return {
        ...
        "normalization_policy": {
            "policy_identifiers_preserved_raw": True,
            "normalized_paths_applied": False,
            "status": "approved_single_code_identifier",
            "reason": "MPS policy identifiers, including dotted identifiers such as IM-33.5, are single code values and are preserved raw.",
        },
        "ingestion_mapping": {
...
def main() -> None:
    ...
    section_inventory = []
    all_maps: list[dict] = parse_listed_map_references(lines)
    all_backlog: list[dict] = spatial_backlog_entries(
        all_maps,
        SectionSpec("front-matter-list-of-maps", "LIST OF MAPS", "LIST OF MAPS", 10, 10, "front_matter"),
    )
